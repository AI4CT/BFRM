...
class GProjection(nn.Module):
    ...
    def __init__(self, mesh_pos, camera_f, camera_c, bound=0, tensorflow_compatible=False, orthographic=True):
    ...
    def forward(self, resolution, img_features, inputs):
        ...
        if self.orthographic:
            # 正交投影: 不考虑z轴的缩放效果
            w = -self.camera_f[0] * positions[:, :, 0] / half_resolution[0] + camera_c_offset[0] / half_resolution[0]
            h = self.camera_f[1] * positions[:, :, 1] / half_resolution[1] + camera_c_offset[1] / half_resolution[1]
        else:
            # 透视投影: 考虑z轴的缩放效果
        ...
        if self.tensorflow_compatible:
            ...
            if not self.orthographic:  # 如果不是正交投影，才需要这一步归一化
                w /= half_resolution[0]
                h /= half_resolution[1]

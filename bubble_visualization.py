...
class BubbleVisualizer:
    ...
    def load_frames(self, frame, csv_dir=None):
        ...
        Args:
            ...
            csv_dir: CSV文件目录路径
        """
        logging.info(f"开始加载帧 {frame} 的气泡")
        ...
        if frame_bubbles:
            ...
            # logging.info(f"帧 {frame} 共加载了 {len(frame_bubbles)} 个气泡")
    ...
    def save_visualization(self, output_dir, frame, color_by='volume', show_edges=False, background_color='white'):
        ...
        # logging.info(f"开始保存帧 {frame} 的可视化结果")
        ...
        plotter.close()
        # logging.info(f"帧 {frame} 的可视化结果已保存到 {output_file}")
...
def main():
    global running
    stl_dir = r"C:\codebase\BFRM\results\RB_bubble_flow2\4-reconstruction\bubble_stl"
    csv_dir = r"C:\codebase\BFRM\results\RB_bubble_flow2\3-3Dbubble_positioning"
    visualizer = BubbleVisualizer(stl_dir)
    bubble_dirs = list(Path(stl_dir).iterdir())
    first_bubble_dir = bubble_dirs[0]
    frame_files = sorted([f for f in first_bubble_dir.glob("*.stl")])
    frame_numbers = [f.stem for f in frame_files]
    render_dir = Path(r"C:\codebase\BFRM\results\RB_bubble_flow2\4-reconstruction\Bubbly_flow_render")
    ...
    if png_files:
        ...
        gif_path = r"C:\codebase\BFRM\results\RB_bubble_flow2\4-reconstruction\bubbly_flow.gif"
